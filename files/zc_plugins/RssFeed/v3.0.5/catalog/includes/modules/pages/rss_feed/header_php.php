 ...
 * Modernized for PHP 8 and bot-resistant delivery by PRO-Webs, Inc., 2026.
 */
...
@define('RSS_CACHE_TIME', '15');
@define('RSS_MAX_ITEMS', '250');
@define('RSS_REQUESTS_PER_MINUTE', '30');

@define('DIR_FS_RSSFEED_CACHE', rtrim(DIR_FS_SQL_CACHE, '/\\') . '/rss-feed');
@define('RSS_ERROR_CACHE_DIR', 'Cache directory not found "' . DIR_FS_RSSFEED_CACHE . '"');


require_once __DIR__ . '/../../../classes/rss_feed.php';
require_once __DIR__ . '/../../../functions/extra_functions/rss_feed_guard.php';

if (!is_dir(DIR_FS_RSSFEED_CACHE)) {
  @mkdir(DIR_FS_RSSFEED_CACHE, 0755, true);
}

$rssRequest = prowebs_rss_normalize_request($_GET, (int)RSS_MAX_ITEMS);
foreach (['feed', 'limit', 'products_id', 'products_model', 'cPath', 'imgsize', 'ref', 'utm_source', 'utm_medium', 'utm_term', 'utm_content', 'utm_campaign'] as $requestName) {
  unset($_GET[$requestName]);
}
$_GET = array_merge($_GET, $rssRequest);
if (!prowebs_rss_rate_limit(DIR_FS_RSSFEED_CACHE, (int)RSS_REQUESTS_PER_MINUTE)) {
  http_response_code(429);
  header('Content-Type: application/rss+xml; charset=UTF-8');
  header('Retry-After: 60');
  echo '<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>Feed request limit reached</title><link></link><description>Please retry in one minute.</description></channel></rss>';
  zen_exit();
}
...
$cacheKey = prowebs_rss_cache_key($rssRequest, (int)$_SESSION['languages_id'], (string)$_SESSION['currency']);
if(!$rss->rss_feed_cache($cacheKey, (int)RSS_CACHE_TIME * 60) && $rss->rss_feed_generation_lock()) {
...
//  $rss->rss_feed_xmlns(array('xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"', 'xmlns:content="http://purl.org/rss/1.0/modules/content/"'));
  ...
  $image = prowebs_rss_channel_image_url((string)RSS_IMAGE);
  if ($image !== false) {
  ...
  $rss->rss_feed_set('managingEditor', prowebs_rss_contact_value((string)RSS_MANAGING_EDITOR));
  $rss->rss_feed_set('webMaster', prowebs_rss_contact_value((string)RSS_WEBMASTER));
  $rss->rss_feed_set('language', $_SESSION["languages_code"]);
  $rssAuthor = prowebs_rss_contact_value((string)RSS_AUTHOR);
  ...
  if(isset($_GET['limit'])) {
    ...
    $limit = " LIMIT " . min((int)RSS_PRODUCTS_LIMIT, (int)RSS_MAX_ITEMS);
  }
  $requestLimit = isset($_GET['limit']) ? (int)$_GET['limit'] : min(max(1, (int)RSS_PRODUCTS_LIMIT), (int)RSS_MAX_ITEMS);
...
*/
        ...
        if(isset($_GET['cPath'])) {
          ...
          if(!$categories->EOF) {
            ...
            if(!isset($_GET['limit']) || (int)$_GET['limit'] >1) {
              zen_rss_category_tree($current_category_id, $cPath, max(0, $requestLimit - 1));
            }
          }
        } else {
          zen_rss_category_tree(0, '', $requestLimit);
        }
      }
      break;
    ...
    case "specials_random":
      $random = true;
      $limit = " LIMIT " . min((int)MAX_RANDOM_SELECT_SPECIALS, (int)RSS_MAX_ITEMS);
    case "specials":
    ...
    case "featured_random":
      $random = true;
      $limit = " LIMIT " . min((int)MAX_RANDOM_SELECT_FEATURED_PRODUCTS, (int)RSS_MAX_ITEMS);
    case "featured":
    ...
    case "best_sellers_random":
      $random = true;
      $limit = " LIMIT " . min((int)MAX_DISPLAY_BESTSELLERS, (int)RSS_MAX_ITEMS);
    case "best_sellers":
      $where_cat = $from_cat = "";
    ...
    case "upcoming_random":
      $random = true;
      $limit = " LIMIT " . min((int)MAX_DISPLAY_UPCOMING_PRODUCTS, (int)RSS_MAX_ITEMS);
    case "upcoming":
      $where_cat = $from_cat = "";
    ...
    case "new_products_random":
      $random = true;
      $limit = " LIMIT " . min((int)MAX_RANDOM_SELECT_NEW, (int)RSS_MAX_ITEMS);
    case "new_products":
...
//IFNULL(, 0)??????
    ...
    if ($random) {
      $products = $db->ExecuteRandomMulti($sql_products, 1);
    } else {
...
//        $xtags['c:price_formatted type="string"'] = $currencies->display_price($products->fields['price'], $cashTaxRate[$products->fields['products_tax_class_id']]);
      ...
      if(RSS_PRODUCTS_QUANTITY == 'true') $xtags['g:availability'] = ((float)$products->fields['products_quantity'] > 0 ? 'in stock' : 'out of stock');
  ...
  function zen_rss_category_tree($id_parent=0, $str_cPath='', $limit = 32767){
    ...
    while(!$categories->EOF && $limit>0) {
      ...
      if (zen_has_category_subcategories($categories->fields['categories_id'])) {
        $limit = zen_rss_category_tree($categories->fields['categories_id'], $new_str_cPath, $limit);
      }
      $categories->MoveNext();
    }
    return $limit;
